...
export default function QuizSetEntryPage() {
        ...
        const fetchQuizData = async () => {
            try {
                ...
                if (status === 200) {
                    ...
                    setQuizStatus(filteredData?.status);
                }
            } catch (err) {
    ...
    const handleDelete = async questionId => {
        try {
            ...
            if (status === 200) {
                //after delete if no question remains we unpublish the quiz set
                if (adminQuestionsData?.Questions?.length === 1) handlePublish();
    ...
    const handlePublish = async () => {
        try {
            const payload = {
                "status": (quizStatus === 'draft') ? 'published' : 'draft',
                "title": adminQuestionsData?.title
            };
            const url = `${import.meta.env.VITE_SERVER_BASE_URL}/admin/quizzes/${quizId}`;
            const { status } = await api.patch(url, payload);
            if (status === 200) {
                setRefreshKey(prev => prev + 1);
                setSingleQuestion(defaultQuestion);
                        ...
                        <div>
                            ...
                            <div className="space-y-4">
                                <div className='flex justify-between'>
                                    ...
                                    <button className={`p-2 font-bold rounded-md ${quizStatus === 'draft' ? 'bg-green-300' : 'bg-red-300'} ${adminQuestionsData?.Questions?.length === 0 ? 'bg-gray-300 cursor-not-allowed' : ''}`} onClick={handlePublish} disabled={adminQuestionsData?.Questions?.length === 0} > {quizStatus === 'draft' ? 'Publish Quiz' : 'Unpublish Quiz'} </button>
                                </div>
                                ...
                                <form onSubmit={isEditing ? handleSubmit(handleUpdateQuestion) : handleSubmit(submitForm)}>
                                    ...
                                    {singleQuestion.options.map((_, idx) => (
                                        <div key={idx} className="flex items-center space-x-2 bg-white rounded-md px-4 py-1 mt-4">
                                            <input
                                                type="radio"
                                                name='selectedOption'
                                                id='selectedOption'
                                    ...
                                    <button type="submit" className="w-full bg-primary text-white p-2 rounded-md mt-6">
